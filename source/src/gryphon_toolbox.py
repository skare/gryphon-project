...
class gryphon_toolbox(object):
    ...
    def setupSolver(self):
        if type(self.f) == type([]):
            ...
            self.n = 1
            if type(d.lhs(self.f).integrals()[0].integrand()) == type(UFL.constantvalue.Zero()): # Not particularly elegant. Should check for another solution.
                self.rank = 1
            else:
                self.rank = 2
    ...
    def printProgress(self,timeEstimate=False,rejectedStep=False):
        ...
        progressBar = "|" + "="*dots + ">" + "."*(n-dots) + "| " + (str(progress)+" %").ljust(7) + (" t=%g"%(self.t)).ljust(15)
        if rejectedStep:
